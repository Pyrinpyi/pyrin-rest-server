...
@app.get("/info/marketcap", response_model=MarketCapResponse | str, tags=["Pyrin network info"])
async def get_marketcap(stringOnly: bool = False):
    """
    Get $PYI price and market cap. Price info is from coingecko.com
    """
    kas_price = await get_kas_price()
    resp = await pyrin_client.request("getCoinSupplyRequest")
    mcap = round(float(resp["getCoinSupplyResponse"]["circulatingLeor"]) / 100000000 * kas_price)
