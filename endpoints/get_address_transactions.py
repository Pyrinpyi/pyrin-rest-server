...
@app.get("/addresses/{pyrinAddress}/transactions",
         response_model=TransactionForAddressResponse,
         response_model_exclude_unset=True,
         tags=["Pyrin addresses"],
         deprecated=True)
@sql_db_only
async def get_transactions_for_address(
        pyrinAddress: str = Path(
            description="Pyrin address as string e.g. "
                        "pyrin:qqpz0ndsjsmlaxmcp33mqrx263dn4n8x9y67gysfkgws8kqa0w9szz6mzdhcq",
    ...
    #
    # WHERE "script_public_key_address" = 'pyrin:qp7d7rzrj34s2k3qlxmguuerfh2qmjafc399lj6606fc7s69l84h7mrj49hu6'
    #
    ...
    async with async_session() as session:
        resp = await session.execute(text(f"""
            ...
            WHERE "script_public_key_address" = :pyrinAddress
			ORDER by transactions.block_time DESC
			LIMIT 500"""),
                                     {'pyrinAddress': pyrinAddress})
...
@app.get("/addresses/{pyrinAddress}/full-transactions",
...
async def get_full_transactions_for_address(
    ...
    async with async_session() as s:
        ...
        tx_within_limit_offset = await s.execute(select(TxAddrMapping.transaction_id)
                                                 .filter(TxAddrMapping.address == pyrinAddress)
                                                 .limit(limit)
                                                 .offset(offset)
...
@app.get("/addresses/{pyrinAddress}/transactions-count",
...
async def get_transaction_count_for_address(
    ...
    async with async_session() as s:
        count_query = select(func.count()).filter(TxAddrMapping.address == pyrinAddress)
